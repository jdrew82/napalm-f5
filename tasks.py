...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAPALM_F5_xxx
namespace = Collection("napalm_f5")
namespace.configure(
    {
        "napalm_f5": {
            "project_name": "napalm_f5",
            "python_ver": "3.14",
            "local": is_truthy(os.getenv("INVOKE_NAPALM_F5_LOCAL", "false")),
            "image_name": "napalm_f5",
            "image_ver": os.getenv("INVOKE_NAPALM_F5_IMAGE_VER", "latest"),
...
def run_command(context, exec_cmd, port=None, rm=True):
    ...
    """
    if is_truthy(context.napalm_f5.local):
        print(f"LOCAL - Running command {exec_cmd}")
        result = context.run(exec_cmd, pty=True)
    else:
        print(
            f"DOCKER - Running command: {exec_cmd} container: {context.napalm_f5.image_name}:{context.napalm_f5.image_ver}"
        )
        if port:
            result = context.run(
                f"docker run -it {'--rm' if rm else ''} -p {port} -v {context.napalm_f5.pwd}:/local {context.napalm_f5.image_name}:{context.napalm_f5.image_ver} sh -c '{exec_cmd}'",
                pty=True,
            )
        else:
            result = context.run(
                f"docker run -it {'--rm' if rm else ''} -v {context.napalm_f5.pwd}:/local {context.napalm_f5.image_name}:{context.napalm_f5.image_ver} sh -c '{exec_cmd}'",
                pty=True,
            )
...
def build(context, cache=True, force_rm=False, hide=False):
    ...
    print(f"Building image {context.napalm_f5.image_name}:{context.napalm_f5.image_ver}")
    command = f"docker build --tag {context.napalm_f5.image_name}:{context.napalm_f5.image_ver} --build-arg PYTHON_VER={context.napalm_f5.python_ver} -f Dockerfile ."
    ...
    if result.exited != 0:
        print(
            f"Failed to build image {context.napalm_f5.image_name}:{context.napalm_f5.image_ver}\nError: {result.stderr}"
        )
...
def clean(context):
    ...
    print(f"Attempting to forcefully remove image {context.napalm_f5.image_name}:{context.napalm_f5.image_ver}")
    context.run(f"docker rmi {context.napalm_f5.image_name}:{context.napalm_f5.image_ver} --force")
    print(f"Successfully removed image {context.napalm_f5.image_name}:{context.napalm_f5.image_ver}")
...
def coverage(context):
    ...
    exec_cmd = "coverage run --source=napalm_f5 -m pytest"
...
def pytest(context, pattern=None, label=None):
    ...
    doc_test_cmd = "pytest -vv --doctest-modules napalm_f5/ tests/"
    pytest_cmd = "coverage run --source=napalm_f5 -m pytest"
    if pattern:
...
def cli(context):
    ...
    Args:
        context (obj): Used to run specific commands
    """
    dev = f"docker run -it -v {context.napalm_f5.pwd}:/local {context.napalm_f5.image_name}:{context.napalm_f5.image_ver} /bin/bash"
...
@task(
    help={
        "version": "Version of napalm_f5 to generate the release notes for.",
